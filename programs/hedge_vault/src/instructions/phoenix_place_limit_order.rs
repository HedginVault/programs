...
pub struct PhoenixLimitOrderParams {
...
impl PhoenixLimitOrderParams {
    fn kind(&self) -> PhoenixOrderKind {
        if self.post_only {
            PhoenixOrderKind::PostOnly
        } else {
            PhoenixOrderKind::Limit
        }
    }

    fn packet(&self) -> OrderPacket {
        let order_flags = OrderPacket::order_flags(self.reduce_only);
...
#[derive(Accounts)]
pub struct PhoenixPlaceLimitOrder<'info> {
    #[account(mut)]
...
impl<'info> PhoenixPlaceLimitOrder<'info> {
    pub fn handler(
        ...
        // a resting order is new risk even when reduce-only, so it is never an exit path
        ...
        // a limit order may partially fill before the rest rests on the book
        ...
        emit!(PhoenixOrderPlaced {
            ...
            side: params.side,
            kind: params.kind(),
            price_in_ticks: Some(params.price_in_ticks),
