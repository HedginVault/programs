...
#[derive(Accounts)]
pub struct ExitStrategyJupiterSwap<'info> {
    #[account(mut)]
...
impl<'info> ExitStrategyJupiterSwap<'info> {
    pub fn handler(
        ...
        let strategy_token_mint_key = source_mint.key();
        ...
        // swapping back to vault deposit mint, so target mint should match source mint
        validate!(
            strategy_target_mint == source_mint.key(),
            HedgeVaultError::InvalidSourceMint
        )?;
